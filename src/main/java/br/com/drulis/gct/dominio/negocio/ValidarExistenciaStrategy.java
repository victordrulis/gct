...
public class ValidarExistenciaStrategy implements StrategyInterface {
	...
	public ValidarExistenciaStrategy() {
    ...
    public String processar(Entidade entidade) {
    	...
    	try {
			return dao.consultar(entidade).size() == 0 ? "Não existem registros para os dados informados." : null;
		} catch (SQLException e) {
			System.out.println("[" + this.getClass().getSimpleName() + "] Erro de validação de existência: " + e.getCause());
			e.printStackTrace();
		}
    	
        return "Erro de validação de existência.";
    }
}
