...
public class AtividadeViewHelper implements ViewHelperInterface {
    ...
    public DominioInterface getData(HttpServletRequest request) {
        ...
        Atividade contato = new Atividade();
    ...
    public void setView(Resultado resultado, HttpServletRequest request, HttpServletResponse response)
        ...
        if(resultado != null) {
            ...
            listAtividade = (List<Atividade>) (Object) resultado.getEntidades();
        }
        ...
        if(listAtividade == null || listAtividade.size() < 1) {
            request.setAttribute("mensagem", Mensagem.ERRO_NAO_ENCONTRADO.getDescricao());
            request.getRequestDispatcher("mensagem.jsp").forward(request, response);
        }
        
        if(listAtividade != null && listAtividade.size() > 1) {
            request.setAttribute("listAtividade", listAtividade);
            request.getRequestDispatcher("list.jsp").forward(request, response);
        }
        
        if(listAtividade != null && listAtividade.size() == 1) {
            request.setAttribute("contato", listAtividade.get(0));
